...
(
    argument0,"
    // Name
    global.rm_name_var = 'Long Hall 5 / Long Hall 6'
    // Spawn
    ...
    local.torch.door_var = global.spawn_arr[1,4];
    // Torch 2
    local.torch = instance_create(208,64,torch_gold_north_obj);
    local.torch.door_var = global.spawn_arr[2,4];
    local.torch = instance_create(208,96,torch_gold_south_obj);
    local.torch.door_var = global.spawn_arr[2,4];
    // 3D Draw
    d3d_start();

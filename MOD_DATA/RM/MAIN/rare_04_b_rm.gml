/*
Argument 0: Room Variable (same for all rooms)
*/
// Spawn spots// Name
room_set_code
(
    argument0,'
    ini_open(global.lang_var);
    global.rm_name_var = ini_read_string("ROOM","phone","ROOM_phone");
    ini_close();
    // Spawn
    global.spawn_len_var = 3;
    global.spawn_arr[0,0] = 160;
    global.spawn_arr[0,1] = 512;
    global.spawn_arr[0,2] = 0;
    global.spawn_arr[0,3] = 0;
    global.spawn_arr[1,0] = 320;
    global.spawn_arr[1,1] = 416;
    global.spawn_arr[1,2] = 0;
    global.spawn_arr[1,3] = 180;
    global.spawn_arr[2,0] = 320;
    global.spawn_arr[2,1] = 608;
    global.spawn_arr[2,2] = 0;
    global.spawn_arr[2,3] = 180;
    // 3D Draw
    d3d_start();
    global.draw_3d_var = true;
    // Doors
    spawn_create_scr(true,false);
');
// Room settings
room_set_width(argument0,1280);
room_set_height(argument0,720);
room_set_background_color(argument0,c_black,true);
room_set_view_enabled(argument0,true);
for (local.i=0; local.i<8; local.i+=1;)
{ room_set_view(argument0,local.i,false,0,0,1280,720,0,0,1280,720,32,32,-1,-1,noone); }
room_set_view(argument0,0,true,0,0,1280,720,0,0,1280,720,32,32,-1,-1,noone);
// Effects
room_instance_add(argument0,0,0,fog_01_obj);
room_instance_add(argument0,0,0,color_control_02_obj);
room_instance_add(argument0,0,0,amb_control_obj);
// Floors
room_instance_add(argument0,160,512,floor_obj);
room_instance_add(argument0,192,512,floor_obj);
room_instance_add(argument0,256,544,floor_obj);
room_instance_add(argument0,192,480,floor_obj);
room_instance_add(argument0,192,544,floor_obj);
room_instance_add(argument0,224,544,floor_obj);
room_instance_add(argument0,224,512,floor_obj);
room_instance_add(argument0,224,480,floor_obj);
room_instance_add(argument0,256,480,floor_obj);
room_instance_add(argument0,256,512,floor_obj);
room_instance_add(argument0,288,480,floor_obj);
room_instance_add(argument0,288,512,floor_obj);
room_instance_add(argument0,288,544,floor_obj);
room_instance_add(argument0,256,448,floor_obj);
room_instance_add(argument0,256,576,floor_obj);
room_instance_add(argument0,256,608,floor_obj);
room_instance_add(argument0,288,608,floor_obj);
room_instance_add(argument0,320,608,floor_obj);
room_instance_add(argument0,256,416,floor_obj);
room_instance_add(argument0,288,416,floor_obj);
room_instance_add(argument0,320,416,floor_obj);
//Ceilings
room_instance_add(argument0,160,512,ceil_obj);
room_instance_add(argument0,192,512,ceil_obj);
room_instance_add(argument0,256,544,ceil_obj);
room_instance_add(argument0,192,480,ceil_obj);
room_instance_add(argument0,192,544,ceil_obj);
room_instance_add(argument0,224,544,ceil_obj);
room_instance_add(argument0,224,512,ceil_obj);
room_instance_add(argument0,224,480,ceil_obj);
room_instance_add(argument0,256,480,ceil_obj);
room_instance_add(argument0,256,512,ceil_obj);
room_instance_add(argument0,288,480,ceil_obj);
room_instance_add(argument0,288,512,ceil_obj);
room_instance_add(argument0,288,544,ceil_obj);
room_instance_add(argument0,256,448,ceil_obj);
room_instance_add(argument0,256,576,ceil_obj);
room_instance_add(argument0,256,608,ceil_obj);
room_instance_add(argument0,288,608,ceil_obj);
room_instance_add(argument0,320,608,ceil_obj);
room_instance_add(argument0,256,416,ceil_obj);
room_instance_add(argument0,288,416,ceil_obj);
room_instance_add(argument0,320,416,ceil_obj);
// Walls (Horizontal)
room_instance_add(argument0,288,464,wall_hor_obj);
room_instance_add(argument0,224,464,wall_hor_obj);
room_instance_add(argument0,192,464,wall_hor_obj);
room_instance_add(argument0,160,496,wall_hor_obj);
room_instance_add(argument0,160,528,wall_hor_obj);
room_instance_add(argument0,192,560,wall_hor_obj);
room_instance_add(argument0,224,560,wall_hor_obj);
room_instance_add(argument0,288,560,wall_hor_obj);
room_instance_add(argument0,288,592,wall_hor_obj);
room_instance_add(argument0,320,592,wall_hor_obj);
room_instance_add(argument0,320,624,wall_hor_obj);
room_instance_add(argument0,288,624,wall_hor_obj);
room_instance_add(argument0,256,624,wall_hor_obj);
room_instance_add(argument0,256,400,wall_hor_obj);
room_instance_add(argument0,288,432,wall_hor_obj);
room_instance_add(argument0,320,432,wall_hor_obj);
room_instance_add(argument0,320,400,wall_hor_obj);
room_instance_add(argument0,288,400,wall_hor_obj);
// Walls (Vertical)
room_instance_add(argument0,240,416,wall_vert_obj);
room_instance_add(argument0,272,448,wall_vert_obj);
room_instance_add(argument0,240,448,wall_vert_obj);
room_instance_add(argument0,336,416,wall_vert_obj);
room_instance_add(argument0,304,480,wall_vert_obj);
room_instance_add(argument0,144,512,wall_vert_obj);
room_instance_add(argument0,176,480,wall_vert_obj);
room_instance_add(argument0,176,544,wall_vert_obj);
room_instance_add(argument0,240,576,wall_vert_obj);
room_instance_add(argument0,240,608,wall_vert_obj);
room_instance_add(argument0,272,576,wall_vert_obj);
room_instance_add(argument0,336,608,wall_vert_obj);
room_instance_add(argument0,304,544,wall_vert_obj);
room_instance_add(argument0,304,512,wall_vert_obj);
// Props
room_instance_add(argument0,292,520,table_obj);
room_instance_add(argument0,240,512,chair_east_obj);
room_instance_add(argument0,292,508,table_obj);
// Phone
room_instance_add(argument0,288,512,phone_trig_obj);
room_instance_add(argument0,292,517,phone_obj);
room_instance_add(argument0,292,508,fan_obj);
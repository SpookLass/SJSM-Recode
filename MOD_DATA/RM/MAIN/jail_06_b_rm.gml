/*
Argument 0: Room Variable (same for all rooms)
*/
// Spawn spots// Name
room_set_code
(
    argument0,'
    ini_open("lang_"+global.lang_var+".ini");
	global.rm_name_var = ini_read_string("ROOM","jail","ROOM_jail")+" 6 B";
	ini_close();
    // Spawn
    global.spawn_len_var = 2;
    global.spawn_arr[0,0] = 48;
    global.spawn_arr[0,1] = 368;
    global.spawn_arr[0,2] = 0;
    global.spawn_arr[0,3] = 0;
    global.spawn_arr[1,0] = 272;
    global.spawn_arr[1,1] = 432;
    global.spawn_arr[1,2] = 0;
    global.spawn_arr[1,3] = 90;
    // Mark
    global.mark_len_var = 3;
    global.mark_arr[0,0] = 150;
    global.mark_arr[0,1] = 319;
    global.mark_arr[0,2] = 0;
    global.mark_arr[1,0] = 124;
    global.mark_arr[1,1] = 373;
    global.mark_arr[1,2] = 0;
    global.mark_arr[2,0] = 267;
    global.mark_arr[2,1] = 387;
    global.mark_arr[2,2] = 0;
    mark_create_scr();
    // 3D Draw
    d3d_start();
    global.draw_3d_var = true;
    // Doors
    spawn_create_scr(true,false);
');
// Room settings
room_set_width(argument0,1280);
room_set_height(argument0,720);
room_set_background_color(argument0,c_black,true);
room_set_view_enabled(argument0,true);
for (local.i=0; local.i<8; local.i+=1;)
{ room_set_view(argument0,local.i,false,0,0,1280,720,0,0,1280,720,32,32,-1,-1,noone); }
room_set_view(argument0,0,true,0,0,1280,720,0,0,1280,720,32,32,-1,-1,noone);
// Effects
room_instance_add(argument0,0,0,fog_01_obj);
room_instance_add(argument0,0,0,color_control_02_obj);
room_instance_add(argument0,0,0,rand_mon_spawn_obj);
room_instance_add(argument0,0,0,amb_control_obj);
// Floors
room_instance_add(argument0,144,304,floor_obj);
room_instance_add(argument0,112,304,floor_obj);
room_instance_add(argument0,176,304,floor_obj);
room_instance_add(argument0,112,336,floor_obj);
room_instance_add(argument0,144,336,floor_obj);
room_instance_add(argument0,176,336,floor_obj);
room_instance_add(argument0,112,368,floor_obj);
room_instance_add(argument0,80,368,floor_obj);
room_instance_add(argument0,48,368,floor_obj);
room_instance_add(argument0,144,368,floor_obj);
room_instance_add(argument0,176,368,floor_obj);
room_instance_add(argument0,208,368,floor_obj);
room_instance_add(argument0,240,368,floor_obj);
room_instance_add(argument0,272,368,floor_obj);
room_instance_add(argument0,272,400,floor_obj);
room_instance_add(argument0,272,432,floor_obj);
// Ceilings
room_instance_add(argument0,144,304,ceil_obj);
room_instance_add(argument0,112,304,ceil_obj);
room_instance_add(argument0,176,304,ceil_obj);
room_instance_add(argument0,112,336,ceil_obj);
room_instance_add(argument0,144,336,ceil_obj);
room_instance_add(argument0,176,336,ceil_obj);
room_instance_add(argument0,112,368,ceil_obj);
room_instance_add(argument0,80,368,ceil_obj);
room_instance_add(argument0,48,368,ceil_obj);
room_instance_add(argument0,144,368,ceil_obj);
room_instance_add(argument0,176,368,ceil_obj);
room_instance_add(argument0,208,368,ceil_obj);
room_instance_add(argument0,240,368,ceil_obj);
room_instance_add(argument0,272,368,ceil_obj);
room_instance_add(argument0,272,400,ceil_obj);
room_instance_add(argument0,272,432,ceil_obj);
// Walls (Horizontal)
room_instance_add(argument0,144,288,wall_hor_obj);
room_instance_add(argument0,112,288,wall_hor_obj);
room_instance_add(argument0,80,352,wall_hor_obj);
room_instance_add(argument0,48,352,wall_hor_obj);
room_instance_add(argument0,208,352,wall_hor_obj);
room_instance_add(argument0,240,352,wall_hor_obj);
room_instance_add(argument0,272,352,wall_hor_obj);
room_instance_add(argument0,80,384,wall_hor_obj);
room_instance_add(argument0,48,384,wall_hor_obj);
room_instance_add(argument0,112,384,wall_hor_obj);
room_instance_add(argument0,144,384,wall_hor_obj);
room_instance_add(argument0,176,384,wall_hor_obj);
room_instance_add(argument0,208,384,wall_hor_obj);
room_instance_add(argument0,240,384,wall_hor_obj);
room_instance_add(argument0,272,448,wall_hor_obj);
room_instance_add(argument0,176,288,wall_hor_obj);
// Walls (Vertical)
room_instance_add(argument0,96,304,wall_vert_obj);
room_instance_add(argument0,96,336,wall_vert_obj);
room_instance_add(argument0,192,336,wall_vert_obj);
room_instance_add(argument0,192,304,wall_vert_obj);
room_instance_add(argument0,288,368,wall_vert_obj);
room_instance_add(argument0,288,400,wall_vert_obj);
room_instance_add(argument0,288,432,wall_vert_obj);
room_instance_add(argument0,256,400,wall_vert_obj);
room_instance_add(argument0,256,432,wall_vert_obj);
room_instance_add(argument0,32,368,wall_vert_obj);
// Torches
room_instance_add(argument0,112,384,torch_south_obj);
room_instance_add(argument0,176,384,torch_south_obj);
// Props
room_instance_add(argument0,112,352,bar_hor_obj);
room_instance_add(argument0,144,352,bar_hor_obj);
room_instance_add(argument0,176,352,bar_hor_obj);
room_instance_add(argument0,176,336,table_rand_obj);
room_instance_add(argument0,121,340,bone_rand_obj);
room_instance_add(argument0,103,338,web_rand_obj);
room_instance_add(argument0,186,322,web_rand_obj);
room_instance_add(argument0,263,370,web_rand_obj);
// Doors
room_instance_add(argument0,96,304,door_west_obj);
room_instance_add(argument0,104,304,door_trig_obj);
room_instance_add(argument0,192,304,door_east_obj);
room_instance_add(argument0,184,304,door_trig_obj);
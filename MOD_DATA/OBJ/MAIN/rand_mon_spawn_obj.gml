// Builtin Variables
object_set_depth(argument0,99);
object_set_mask(argument0,noone);
object_set_parent(argument0,noone);
object_set_persistent(argument0,false);
object_set_solid(argument0,false);
object_set_sprite(argument0,noone);
object_set_visible(argument0,false);
// Create
object_event_add // ev_create,0
(argument0,ev_other,ev_room_start,'
    ds_list_clear(mon_curr_list);
    local.mons = 0;
    local.nospawn = global.no_mon_var;
    with mon_par_obj
    {
        ds_list_add(mon_curr_list,id);
        local.mons += 1;
        if intro_var || boss_var
        { local.nospawn = true; }
    }
    local.mult = get_mult_scr();
    if (local.mons < local.mult || local.mult < 0) && !local.nospawn
    {
        if global.count_var <= 0
        {
            if global.mult_type_var == 3 // Hardest
            {
                ds_list_clear(mon_spawn_list);
                ds_list_copy(mon_spawn_list,mon_list);
                for (local.i=0; local.i<ds_list_size(mon_curr_list); local.i+=1;)
                {
                    local.mon = ds_list_find_value(mon_curr_list,local.i);
                    if global.dupe_var == dupe_never_const || local.mon.dupe_var == dupe_never_const
                    || (global.dupe_var == dupe_canon_const && local.mon.dupe_var != dupe_canon_const)
                    {
                        local.index = ds_list_find_index(mon_spawn_list,local.mon.object_index);
                        if local.index != -1 { ds_list_delete(mon_spawn_list,local.index); }
                    }
                }
                local.size = ds_list_size(mon_spawn_list);
                while local.size > 0 && (local.mons < local.mult || (local.mult < 0 && local.mons < 5))
                {
                    local.mon = instance_create(0,0,ds_list_find_value(mon_spawn_list,irandom(local.size-1)));
                    ds_list_add(mon_curr_list,local.mon);
                    if global.dupe_var == dupe_never_const || local.mon.dupe_var == dupe_never_const
                    || (global.dupe_var == dupe_canon_const && local.mon.dupe_var != dupe_canon_const)
                    {
                        local.index = ds_list_find_index(mon_spawn_list,local.mon.object_index);
                        if local.index != -1 { ds_list_delete(mon_spawn_list,local.index); }
                        local.size = ds_list_size(mon_spawn_list)
                    }
                    local.mons += 1;
                    if global.reset_spd_var > 0 && global.game_spd_var > 1
                    { global.game_spd_var = 1; fmod_group_set_pitch_scr(0,global.game_spd_var); }
                }
                global.count_var = get_count_scr();
            }
            else
            {
                if !irandom(global.mon_chance_var+(global.mon_chance_mult_var*local.mons)-1-global.mon_fail_var)
                {
                    ds_list_clear(mon_spawn_list);
                    ds_list_copy(mon_spawn_list,mon_list);
                    for (local.i=0; local.i<ds_list_size(mon_curr_list); local.i+=1;)
                    {
                        local.mon = ds_list_find_value(mon_curr_list,local.i);
                        if global.dupe_var == dupe_never_const || local.mon.dupe_var == dupe_never_const
                        || (global.dupe_var == dupe_canon_const && local.mon.dupe_var != dupe_canon_const)
                        {
                            local.index = ds_list_find_index(mon_spawn_list,local.mon.object_index);
                            if local.index != -1 { ds_list_delete(mon_spawn_list,local.index); }
                        }
                    }
                    local.size = ds_list_size(mon_spawn_list);
                    if local.size > 0
                    {
                        local.mon = instance_create(0,0,ds_list_find_value(mon_spawn_list,irandom(local.size-1)));
                        ds_list_add(mon_curr_list,local.mon);
                        global.count_var = get_count_scr();
                        global.mon_fail_var = 0;
                        if global.reset_spd_var > 0 && global.game_spd_var > 1
                        { global.game_spd_var = 1; fmod_group_set_pitch_scr(0,global.game_spd_var); }
                    }
                }
                else { global.mon_fail_var += 1; }
            }
        }
        else { global.count_var = max(0,global.count_var-1); }
    }
    ds_list_sort(mon_curr_list,true);
');
// Try summon
object_event_add
(argument0,ev_other,ev_user0,'
    show_message("Whos doing that?");
    ds_list_clear(mon_spawn_list);
    ds_list_copy(mon_spawn_list,mon_list);
    for (local.i=0; local.i<ds_list_size(mon_curr_list); local.i+=1;)
    {
        local.mon = ds_list_find_value(mon_curr_list,local.i);
        if global.dupe_var == dupe_never_const || local.mon.dupe_var == dupe_never_const
        || (global.dupe_var == dupe_canon_const && local.mon.dupe_var != dupe_canon_const)
        {
            local.index = ds_list_find_index(mon_spawn_list,local.mon.object_index);
            if local.index != -1 { ds_list_delete(mon_spawn_list,local.index); }
        }
    }
    local.size = ds_list_size(mon_spawn_list);
    if local.size > 0
    {
        local.mon = instance_create(0,0,ds_list_find_value(mon_spawn_list,irandom(local.size-1)));
        ds_list_add(mon_curr_list,local.mon);
        global.count_var = get_count_scr();
        global.mon_fail_var = 0;
        if global.reset_spd_var > 0 && global.game_spd_var > 1
        { global.game_spd_var = 1; fmod_group_set_pitch_scr(0,global.game_spd_var); }
    }
');